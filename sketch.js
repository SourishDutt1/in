...
var endline,canvas;
function preload() {
  spaceImage = loadImage("backgr.png");
  ...
  bossImage=loadImage("boss 2.png");
...
function setup() {  
  ...
  boss=createSprite(canvas.width/2,canvas.height-canvas.height-300);
  ...
  //p1.debug = true;
  p1.setCollider("circle",spaceShip.x,spaceShip.y,50);
  p1.visible = false;
  p2 = createSprite(250,600); 
  p2.setCollider("circle",spaceShip.x,spaceShip.y,50);
  //p2.debug = true;
  p2.visible = false;
  ...
  endline = createSprite(500,700,500,5);
  endline.visible = false;
  line = createSprite(500,300,500,5);
  line.visible = false;
}
...
function draw() {
  ...
  if(gameState === play) {
    ...
    if(keyDown("space") ) {
      if(frameCount%10===0){
      laser = createSprite(spaceShip.x,spaceShip.y-50);
...
function asteroids() {
  if(frameCount % 110 === 0) {
    ...
    var asteroid = createSprite(Math.round(random(50,950)),-20);
...
function missile1() {
  if(frameCount % 30 === 0) {
    ...
    var missile1 = createSprite(boss.y-200,-20);
    missile1.velocityY = 4
    missile1.lifetime = 200;
    missile1.scale = random(0.1,0.3);
    //asteroid.debug = true;
